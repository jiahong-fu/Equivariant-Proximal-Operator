...
class Fconv_PCA(nn.Module):
    ...
    def forward(self, input):
        ...
        output = F.conv2d(input, _filter,
                        ...
                        dilation=1,
                        groups=1)
        # print("!!!Fconv_PCA #############: ", output[:, :, 0:180, 0:180].max(), output[:, :, 0:180, 0:180].min())
        # print("***Fconv_PCA #############: ", output[:, :, -180:, -180:].max(), output[:, :, -180:, -180:].min())
        return output + _bias
...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        res += x
        # print("!!!Resblock #############: ", res[:, :, 0:180, 0:180].max(), res[:, :, 0:180, 0:180].min())
        # print("***Resblock #############: ", res[:, :, -180:, -180:].max(), res[:, :, -180:, -180:].min())
        return res
